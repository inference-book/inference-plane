...
// EnvHTTPDebug enables the HTTP debug transport when set to any non-empty
// value. Off by default; the v0.1 example's Makefile flips it on so the
...
// io.ReadCloser. The memory cost is fine for debugging; the production
// path leaves debugTransport disabled.
