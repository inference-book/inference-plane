...
|---------|------------------------------------------|----------------------------------------------------------------------------------------------------|
| Ch 7    | Data plane: router → queue → scheduler   | Three beats forming the chapter's act sequence. **Beat 1 — Router:** iplane sits in front of the engine. New OpenAI-compatible HTTP server at `iplane serve :8080` does path-based deployment lookup (`/v1/<deploy-id>/chat/completions` → look up engine endpoint from state → forward → instrument). Operator-facing URL becomes `http://<iplane>/v1/<deploy-id>/...` instead of the engine proxy URL; the engine URL becomes an internal implementation detail. Request-level metrics (`iplane_chat_requests_total`, `iplane_chat_request_duration_seconds`, `iplane_chat_completion_tokens_total`) tick at the router, restoring the dashboard panels removed in PR 52. Both `iplane serve` and `iplane up` host the router (in-process for `up`). **Beat 2 — Queue + priority scheduler:** the existing "first store appears (`internal/stores/`)" framing. The router gains a buffer in front; scheduler picks the next request to dispatch (fairness lanes, per-tenant weights, interactive vs. batch class). **Beat 3 — Multi-replica fan-out:** with router + queue in place, fanning to N replicas of the same model is a natural extension; sets up Ch 8's prefix-cache-across-replicas narrative. Each beat is a separate PR series; the router (Beat 1) is the prerequisite for the other two. |
