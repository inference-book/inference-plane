...
| All     | Vast.ai + AWS adapters           | Provider mix becomes real; cost-aware routing has providers to choose between. Cross-provider warm-cache design + the per-provider volume-create matrix is in [docs/design/0004-cross-provider-warm-cache.md](docs/design/0004-cross-provider-warm-cache.md); AWS/GCP are the clean home, and Lambda's filesystem-create was recorded as API-blocked on a 405 that turns out to have been fired at a path with no POST (#432). |
