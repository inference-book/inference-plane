...
| 2 | Deploy primitive                         | active | `iplane deployment deploy --image <engine> --model <ref>` -- image-as-pod via the new `provisioners.Deployer` capability (PR 44). RunPod's Deployer spawns a pod whose container IS the engine image; engine `/health` is HTTP-polled from the operator side. VM-style providers (Lambda Labs in v0.2, raw AWS) fall back to the `sshdocker.Executor` via the Service's capability dispatch -- both paths satisfy the same `Deployer` interface. PR B (demo update + collapse instance/deployment for image-native providers) outstanding. |
| 3 | Telemetry seeding                        | designed | Cp boots cloudflared quick tunnel (lab default) or accepts `--otel-endpoint <url>`; deploy passes `OTEL_EXPORTER_OTLP_ENDPOINT` to data plane container. |
| 4 | `ModelStore` interface + HF impl         | designed | `internal/modelstores/`, `huggingface/` impl. Interface seeded for v0.2's `RunPodVolumeStore` + `CachedStore` wrapper. |
| 5 | `iplane up` UX wrapper                   | designed | Chains 1→2→3→4 in one command. The chapter's promised "API key in, OpenAI endpoint out" flow.       |
...
|---------|------------------------------------------|----------------------------------------------------------------------------------------------------|
| Ch 7    | Request queue + priority scheduler       | Cp-side queue feeding the engine batcher. First store appears (`internal/stores/`).                 |
