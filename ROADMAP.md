...
| 2 | Deploy primitive                         | designed | `iplane deploy --instance <id> --image <ref> --model <ref>`. SSH `docker run` over the provisioned base image. |
...
- **Provisioner / deploy split on RunPod.** Phase 1 provisions with a generic Docker-capable base image; phase 2 does `docker run` over SSH. Slower than RunPod-native container-at-creation, but keeps the abstraction portable to AWS/Lambda.
