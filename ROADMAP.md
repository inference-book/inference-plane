...
| 4 | `ModelStore` interface + HF impl         | done | `internal/modelstores/` (interface + Passthrough fallback) + `internal/modelstores/huggingface/` (HF API client). v0.1 use is **pre-flight validation**: `Service.CreateDeployment` calls `ModelStore.Resolve(spec)` before any provider touch; HF impl GETs `huggingface.co/api/models/<id>` with a 5s timeout. Catches typos (404), gated-access (401/403 with HF_TOKEN hints), disabled models, network errors (with bypass flag hint). Propagates `$HF_TOKEN` env onto the pod for gated-model auth. New persistent `--skip-model-validation` root flag (and `IPLANE_SKIP_MODEL_VALIDATION=1` env fallback) for offline / firewalled / non-HF model setups; default is on. No proto changes — the layer is Service-internal. Interface seeded so the Ch 9 warm cache wraps `huggingface.Store` without touching Service code — which it did, shipping generic as `internal/modelstores/volumecache` (not the earlier-sketched `CachedStore`/`RunPodVolumeStore` names). See [docs/modelstore.md](docs/modelstore.md) and the Ch 9 warm-cache row below. |
...
**Chapters:** Ch 7 (Routing, Queueing, Replicas), Ch 8 (Stateful Routing and Prefix-Cache Reuse), Ch 9 (Scaling to 30B), Ch 10 (Multi-GPU and Distributed Inference)
**Branch:** `release/v0.2` · **Tags:** `ch07-final`, `ch08-final`, `ch09-final`, `ch10-final`
...
| Ch 8    | Prefix-cache affinity + engine-stats-aware routing | The chapter that makes the engine-side prefix cache *pay off across replicas*. Ch 7 Beat 3 fanned a deployment out to N replicas behind round-robin; round-robin spreads load evenly and in doing so **defeats** the engine's prefix cache (turn 2 of a chat lands on a different engine than turn 1, re-prefilling from cold). Resolving that tension is the chapter. Three threads. **Thread 1 — `PrefixAffinity` policy (headline):** the second `internal/router/policy.Policy` impl behind the seam PR 148 cut (the package doc already names it as the Ch 8 successor to `RoundRobin`). Affinity concentrates a session's turns on the replica whose prefix is warm. The router maintains its **own approximate prefix→replica map from routing history** — it *predicts* cache residency from what it routed, it does **not** query the engine (the engine can't cheaply report a prefix-to-replica map anyway). Ties between equally-hot replicas break on `Stats.InFlight`, the method the seam already exposes for exactly this. Two affinity keys = two distinct cache-reuse phenomena: explicit per-session stickiness (`X-IPlane-Session` header — a **request-level hint** like Ch 7's `X-IPlane-Tenant`/`X-IPlane-Priority`; affinity is a traffic property, never a `Deployment` field) for the multi-turn chat / agentic case; implicit longest-shared-prefix hashing for the shared-system-prompt case across distinct sessions. Session-sticky is the load-bearing primitive; prefix-hash layers on top. **Thread 2 — load-aware tie-breaking (start minimal, resist the eager build):** affinity must not pin so hard it overloads a hot replica, so ties need a load signal. **Start with what the router already owns** — the per-`(deploy, replica)` in-flight counts from `inflight.go` (#88), already on `Stats`. Only if the demo *actually* shows naive affinity saturating a replica do we add a **direct, minimal** read of the engine's own saturation signal (vLLM `gpu_cache_usage_perc` / `num_requests_waiting`) on the routing path — **pull, not a pushing agent**, and only the one or two fields the policy consumes. Two consumers of engine metrics split by latency budget: **routing decisions** read a thin direct signal (the data plane's correctness must not depend on the obs pipeline being up); **observability** (hit-rate panels, issue 51) routes through the OTel collector. **Cross-engine standardization is deferred and config-side, not a hand-rolled Go adapter registry:** when a second engine (SGLang, TGI — different metric names / `sglang:` vs `vllm:` prefixes) appears, normalize in an **OTel collector `transform`/OTTL processor on the scrape path** (`prometheusreceiver` → rename to a canonical vocabulary), keeping iplane out of the business of memorizing every engine's metric surface. Pluggable-model-server stays intact. Closes the observability half of issue 51, parked in `docs/telemetry.md` for "when the queue puts iplane back in the data path." **Thread 3 — prefix-cache observability (the original Ch 8 scope, kept):** cache hit-rate metric family at both layers — the engine's `gpu_prefix_cache_hit_rate` (**still deferred to issue 51**, not scraped -- see the shipped note below and the `ch08-final` snapshot) and iplane's routing-decision view (affinity-hit vs affinity-miss). Money demo (`examples/07-prefix-affinity/`): same multi-turn workload, `--routing-policy round_robin` vs `prefix_affinity`, watch hit-rate and TTFT diverge. **Explicitly deferred to Ch 10 (Multi-GPU and Distributed Inference):** the engine's KV domain spanning **multiple GPUs** via tensor / pipeline parallelism. Ch 8 treats each engine as **one opaque KV domain = one endpoint**; iplane routes to *engines, not GPUs*, and never looks inside the TP group. The "engine owns N GPUs in one sharded KV cache" mechanics are Ch 10's lead. The **push-based engine agent / control-channel registration** is likewise Ch 10's (`CP tracks N data planes via control-channel registration`), not Ch 8 — Ch 8's data needs are satisfied by pull. |
| Ch 9    | Quantization-aware deploy                | Image catalog gains AWQ/GPTQ variants. Class shifts to "medium". `iplane deploy --quantization awq` did not land here: quantization reaches the engine only as an opaque `--engine-args` string, and the flag's second job is telling the VRAM budget what to compute, so it moved to Ch 12 with the pre-rent refusal (#325). |
| Ch 9    | Warm-cache model pinning (distance reduction) | **done, PRs 197–201 (issues 190/191/192).** The cold-start-distance wedge: pre-stage a model onto a provider volume so deploys mount weights instead of re-downloading from HF. Shipped generic (no `RunPodVolumeStore` name survived — the store is `internal/modelstores/volumecache`, provider-agnostic). **Mount seam (#197):** `Service` stamps `Resolved.Mounts` onto a provider-agnostic `VolumeMount{volume_id, host_path, mount_path, provider}`; RunPod adapter maps it onto `networkVolumeId`, sshdocker onto `docker run -v`. **Cold-start SLI (#198):** `storage_tier` (cold\|warm) label on the deploy phase/provision metrics + "Engine-init: warm vs cold" Grafana panel, derived from `dep.Mounts` in the deploy observer (adapters stay OTel-free, CP/DP-1). **Pin registry (#191a):** `Volume` proto + `State.Volumes` (schema 1.6, additive), the `VolumeManager` optional provider capability (RunPod network-volume CRUD + `StageModel` CPU setup-pod; Lambda filesystems joined in PR 444, staging still owed on #436), and the cross-provider mount guard. **Verbs (#191c):** `iplane model pin/ls/unpin` (in-process; pin-before-serve). **Auto-resolve (#191b):** deploy joins `(model, provider, region)` → pinned volume from the registry, no config. **Live-validated end-to-end 2026-07-29** on a 72B FP8 flagship (single B200): cold 2692s → warm 244s, ~11x, ~$4.41 → ~$0.40. That pass hardened the RunPod adapter (SKU catalog, container-disk sizing, three aligned timeouts) and **changed the staging completion mechanism** from the earlier `desiredStatus` polling (RunPod never reports a one-shot command's exit) to reading a marker from the **v2 pod-logs stream**; download uses `hf download --max-workers 2` (`huggingface-cli` is gone; 8 workers OOM the ~4 GB staging pod on 70B+). See CLAUDE.md gotchas. **Deferred:** remote `--service-url` pin; `S3Store`/`GCSStore` object-store staging (v0.3+ row below). |
| Ch 10   | Networking-aware provisioning + tracked distributed fleet | **Largely shipped 2026-08.** Engine agent + injected identity (#232-#237), `SERVING_DEGRADED` made reachable (#238), `examples/09-multi-gpu` walkthrough (#239), TTFT + SSE in `mock-engine` (#240), interconnect health as a first-class signal (#213, PR 269), and the fabric A/B harness (#215 partial, PR 261). Provider work the chapter needed landed alongside: Vast implemented as image-native (#252), marketplace host-quality floors (#255), the 40 GB A100 tier made requestable (#257), requesting the *absence* of fabric (#258), and terminal-failure detection hoisted to a Provider capability (#256/#265). **Still open:** the paid NVLink-vs-PCIe A/B measurement itself (#215), multi-node pools (#212), failure attribution (#214). Moved here from v0.3 to match `RELEASE.md` (v0.2 = Ch 7–10). Reframed 2026-07 to a **control-plane** angle: the engine stays the opaque box the book has kept since Ch 8, and iplane's two jobs are renting the right pool and tracking what runs on it. **Networking-aware provisioning (#203):** interconnect metadata per SKU plus a `--needs-nvlink` / `--needs-rdma` provisioning filter, extending the hardware-spec-on-`Instance` seam PR 156 cut so the provisioner selects only matching SKUs. **Push-based engine agent (#204):** control-channel registration, the pull-to-push move Ch 8 deliberately deferred, so the CP tracks N distributed data planes instead of polling them. **Fleet verbs (#205):** `iplane fleet status` / `iplane fleet drain` over the tracked fleet; depends on 204. The multi-GPU KV domain (one engine's cache sharded across cards, logically one, physically many) is taught as a *concept* at the quantization altitude, enough to reason about NVLink/RDMA bandwidth as the binding constraint. Tensor and pipeline parallelism are engine concerns iplane provisions *for*, not internals this repo re-implements; the `iplane deploy --tp 4 --pp 2` flags stay in Ch 12's row. |
| All     | Lambda Labs adapter                      | Proves the Provisioner interface beyond N=1 (provider-side; `local` already proves N=2). Validated against live hardware 2026-08-24, which cost $0.86 and found seven defects, three of them on the shared VM-style path rather than in the adapter. See [docs/design/0010](docs/design/0010-lambda-validation-findings.md). |
...
- **Networking-aware provisioning.** Capability flags on instance requests: `--needs-nvlink`, `--needs-rdma`. Provisioner selects only matching SKUs. Moved from v0.3 with Ch 10; tracked as issue 203.
...
- **(Ch 10) Control-channel protocol shape: long-lived gRPC stream vs. periodic registration.** Decide before Ch 10 (issue 204). Moved from v0.3 with the chapter.
...
**Shipped — Ch 8 complete** (epic #176 closed). (1) session mode — PR #177. (2) `PrefixAffinity` + `router.routing_policy` toggle — PR #178. Local GPU-free harness (external non-owning provider + `iplane mock-engine`), split out as its own step — PR #183. (3) affinity hit-rate metric + Grafana panel, measured router-side as a proxy for the engine's `gpu_prefix_cache_hit_rate` (the engine metric itself is deferred to issue 51, not faked on mock) — PR #184. (4) load-aware override (`affinity_overload_threshold`) — PR #185. (5) header-less body-derived session key (flat-URL only, opening-hash) — PR #186. (6) `examples/07` walkthrough (07a/07b/07c, GPU-free) — PR #188. Non-code remainder: the paid multi-replica GPU run for the book's hit-rate/latency figures (checklist in `examples/07-prefix-affinity/README.md`). Backlog: HostedAPI provider (#182, Ch 11), demo-fidelity driver features (#187).
...
| All     | Vast.ai + AWS adapters           | Provider mix becomes real; cost-aware routing has providers to choose between. Cross-provider warm-cache design + the per-provider volume-create matrix is in [docs/design/0004-cross-provider-warm-cache.md](docs/design/0004-cross-provider-warm-cache.md). AWS/GCP are the clean home, and Lambda turns out to have API-creatable filesystems after all: the 405 that wrote them off was fired at the hyphenated read path, and `POST /api/v1/filesystems` works (#432, probed live 2026-08-24). |
...
The rule cuts the other way too: a capability that *can* be tried and has not
been is a claim on credit. `internal/provisioners/lambdalabs` shipped done,
passed its unit tests for months, and could not have completed a single deploy,
because the key the VM booted with and the key iplane presented were never the
same key. Reading found three defects and the rental found four more, and the
worst of them was that a serving deployment could not be destroyed at all.
Forty minutes and $0.86 (#427, [0010](docs/design/0010-lambda-validation-findings.md)).

The transferable part is the ratio. Three of the four rentals went on finding
defects rather than on the plan, and three of the seven defects were in the
shared path rather than the adapter, so they had been waiting for whichever
VM-style provider got exercised first. AWS (#428) would have hit the same three.
