...
| `iplane model`       | `pin` / `ls` / `unpin` — warm-cache pin registry (Ch 9). `pin <model> --provider --region` pre-stages weights onto a provider volume so deploys mount instead of re-downloading. `pin`/`unpin` are in-process and need the state lock, so pin **before** `iplane serve`; `ls` reads, takes no lock, and honours `--service-url` (#307). See [docs/modelstore.md](docs/modelstore.md) |
