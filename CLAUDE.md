...
- **RunPod setup-pod status strings are unvalidated.** `internal/provisioners/runpod/stage.go`'s `StageModel` polls the throwaway download pod's `DesiredStatus` and treats `EXITED/STOPPED/TERMINATED/COMPLETED` as done, `FAILED` as error — isolated behind `isStageComplete`/`isStageFailed` and marked `// VALIDATE`. The poll/terminate flow is fake-harness tested, but the exact strings a completed `dockerStartCmd` reports (and RunPod CPU-pod create field specifics) need confirming against a live account before the staging demo relies on them.
