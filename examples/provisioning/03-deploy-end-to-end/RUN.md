...
sequenceDiagram
    ...
    Note over Operator,Engine: Step 6: Serve a real inference request
...
Provider:      runpod
Deployment id: demo-llama-20260525t213638 (the instance shares this id)
...
CLI form:
  iplane deployment deploy demo-llama-20260525t213638 --provider runpod --class small --image vllm/vllm-openai:v0.7.0 --model <chosen> --service-url http://localhost:9091
...
CLI form (no native verb; uses the engine_endpoint from `iplane deployment describe`):
  endpoint=$(iplane deployment describe demo-llama-20260525t213638 --service-url http://localhost:9091 -o json | jq -r .engine_endpoint)
  curl -fsS "${endpoint}/v1/models"

### Step 6: Serve a real inference request

The chapter's payoff. Single-turn /v1/chat/completions: prompt in, tokens out. iplane is NOT in the data path -- this POST goes from the operator's laptop straight to the engine_endpoint (the provider's HTTPS proxy URL). What iplane delivered was 'a dialable endpoint serving an OpenAI-compat API'; this step proves the operator can use it.

CLI form:
  iplane deployment query demo-llama-20260525t213638 "Say hello in one sentence." --service-url http://localhost:9091
...
CLI form:
  iplane deployment destroy demo-llama-20260525t213638 --service-url http://localhost:9091
