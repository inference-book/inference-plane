...
- **[09-multi-gpu/](09-multi-gpu/)** — Ch 10 closer. A four-card group and a single card in one fleet, watched through the control channel as the group assembles, loses a link, and stops answering: `assembling` (invisible to any health poller, because during assembly there is no endpoint to ask), the span column carrying `4c/1n` next to `1c/1n`, `serving, link down` (correct tokens and a dead link at once), and `lost` when the lease expires. GPU-free (two `iplane mock-engine --register` processes), so it costs nothing. The span is fabricated and the link failure is simulated; every state transition is real. The NVLink-vs-PCIe throughput A/B needs real pools and is a separate paid run.
